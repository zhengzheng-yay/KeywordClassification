...
class ResidualBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.half_receptive_fields != 0:
            inputs = inputs[:, :, self.half_receptive_fields:-self.half_receptive_fields]
...
class MultiScaleTCN(nn.Module):
    ...
    def normalize_length(self, skip_connections):
        ...
        for x in skip_connections:
            ...
            if remove_length != 0:
                normalized_outputs.append(x[:, :, remove_length:-remove_length])
            else:
                normalized_outputs.append(x)
        return normalized_outputs

    def forward(self, x, length):
        outputs = F.pad(x, (0, 0, self.half_receptive_fields, self.half_receptive_fields, 0, 0), 'constant')
        #pdb.set_trace()
